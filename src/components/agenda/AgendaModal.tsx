...
export function AgendaModal({ onClose, onSave }: { onClose: () => void; onSave: (item: Partial<AgendaItem>) => Promise<unknown> }) {
  const [item, setItem] = useState<Partial<AgendaItem>>({ dia: 'Lunes', turno: 'Manana', desde: '09:05', hasta: '09:55', actividad: 'Glifing', tipoDispositivo: 'Touch', cantidad: 1, estado: 'Pendiente' });
    ...
    <Modal title="+ Nueva actividad" onClose={onClose}>
      <form className="stack" onSubmit={async event => {
        ...
        <div className="grid-2">
          <label>Dia<select className="input" value={item.dia} onChange={e => update('dia', e.target.value)}><option>Lunes</option><option>Martes</option><option>Miercoles</option><option>Jueves</option><option>Viernes</option></select></label>
          <label>Turno<select className="input" value={item.turno} onChange={e => update('turno', e.target.value)}><option>Manana</option><option>Tarde</option></select></label>
        </div>
        <div className="grid-2">
        ...
        <label>Ubicacion<input className="input" value={item.ubicacion || ''} onChange={e => update('ubicacion', e.target.value)} /></label>
