...
export function Credentials365Tool() {
  ...
  useEffect(() => {
    fetchToolsConfig()
      .then(c => {
        setModoPrueba(c.modoPrueba);
        setMailOk(Boolean(c.mailConfigurado ?? c.smtpConfigurado));
        setResendOk(Boolean(c.resendConfigurado));
      })
      .catch(() => {});
  }, []);
  ...
  const onFile = async (file: File | null) => {
    ...
    try {
      ...
      if (!r.ok) { setError(r.error || 'CSV invalido'); return; }
      setPreview(r.preview || []);
      setTotal(r.total || 0);
      setInfo(`${r.total} fila(s) detectadas. Genera la vista previa para validar emails.`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const onPreview = async () => {
    ...
    try {
      ...
      setTotal(r.total || 0);
      setValidos(r.validos || 0);
      setInvalidos(r.invalidos || 0);
      setPreview(r.preview || []);
      setInvalidPreview(r.invalidPreview || []);
      setModoPrueba(Boolean(r.modoPrueba));
      setInfo(`Vista previa lista. ${r.validos} validos - ${r.invalidos} invalidos.`);
    } finally {
      setBusy(false);
    }
  };
  ...
  const providerLabel = resendOk ? 'Resend configurado' : (mailOk ? 'Mail configurado' : 'Mail sin configurar');
  ...
  return (
    <section className="card tool-card">
      ...
      <div className="tool-format">
        ...
        <code>{REQUIRED.map(displayColumn).join(',')}</code>
      </div>
      ...
      <div className="actions wrap-actions">
        ...
        <Button variant="primary" onClick={onSend} disabled={!csv || busy || (!modoPrueba && !mailOk)}>
      ...
      {(total > 0) && (
        <div className="tool-counters">
          ...
          <div className="ok"><span>Validos</span><strong>{validos}</strong></div>
          <div className="bad"><span>Invalidos</span><strong>{invalidos}</strong></div>
          {stats && <>
      ...
      {preview.length > 0 && (
        <div className="table-wrap" style={{ marginTop: 12 }}>
          <table>
            <thead><tr>{REQUIRED.map(c => <th key={c}>{displayColumn(c)}</th>)}</tr></thead>
            <tbody>
              {preview.map((row, idx) => (
      ...
      {invalidPreview.length > 0 && (
        <div className="table-wrap" style={{ marginTop: 12 }}>
          <h4 className="muted" style={{ padding: '8px 12px 0' }}>Filas invalidas</h4>
          <table>
