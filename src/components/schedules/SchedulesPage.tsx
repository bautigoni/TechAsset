...
const EMPTY_SCHEDULE: Partial<TeacherScheduleEntry> = { teacher: '', course: '', subject: '', room: '', dayOfWeek: 1, startTime: '08:00', endTime: '08:45' };
...
export function SchedulesPage({ consultationMode }: { consultationMode: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(item => [item.teacher, item.course, item.subject, item.room, DAY_NAMES[item.dayOfWeek]].some(value => String(value || '').toLocaleLowerCase('es').includes(needle)));
  }, [items, search]);
  ...
  return (
    <section className="view active schedules-page">
      ...
      {tab === 'teachers' ? (
        <>
          {current.length > 0 && <div className="schedule-now-grid">{current.map(item => <div className="schedule-now card" key={item.id}><span>Ahora</span><strong>{item.teacher}</strong><p>{item.course}{item.subject ? ` · ${item.subject}` : ''}</p><small>{item.startTime}–{item.endTime}{item.room ? ` · ${item.room}` : ''}</small></div>)}</div>}
          <div className="schedule-controls card">
            ...
            <Button variant="primary" disabled={consultationMode} onClick={() => setEditing({ ...EMPTY_SCHEDULE })}>+ Agregar horario</Button>
          </div>
          <div className="schedule-groups">
            ...
            {!grouped.length && <div className="empty-state">No hay horarios que coincidan.</div>}
          </div>
        </>
      ) : (
      ...
      {editing && <Modal title={editing.id ? 'Editar horario' : 'Nuevo horario'} onClose={() => setEditing(null)}><form className="stack" onSubmit={saveSchedule}><label>Docente<input className="input" required value={editing.teacher || ''} onChange={event => setEditing(value => ({ ...value!, teacher: event.target.value }))} /></label><label>Curso<input className="input" required value={editing.course || ''} onChange={event => setEditing(value => ({ ...value!, course: event.target.value }))} /></label><div className="grid-2"><label>Materia<input className="input" value={editing.subject || ''} onChange={event => setEditing(value => ({ ...value!, subject: event.target.value }))} /></label><label>Aula<input className="input" value={editing.room || ''} onChange={event => setEditing(value => ({ ...value!, room: event.target.value }))} /></label></div><label>Día<select className="input" value={editing.dayOfWeek || 1} onChange={event => setEditing(value => ({ ...value!, dayOfWeek: Number(event.target.value) }))}>{DAY_NAMES.slice(1).map((name, index) => <option value={index + 1} key={name}>{name}</option>)}</select></label><div className="grid-2"><label>Desde<input className="input" type="time" required value={editing.startTime || ''} onChange={event => setEditing(value => ({ ...value!, startTime: event.target.value }))} /></label><label>Hasta<input className="input" type="time" required value={editing.endTime || ''} onChange={event => setEditing(value => ({ ...value!, endTime: event.target.value }))} /></label></div><div className="actions"><Button variant="primary" type="submit">Guardar</Button><Button type="button" onClick={() => setEditing(null)}>Cancelar</Button></div></form></Modal>}
    </section>
  );
}
