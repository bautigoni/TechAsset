...
import { getDeviceOverview, type DeviceOverviewResponse } from '../../services/devicesApi';
...
export function DeviceProfile({ device, onClose }: { device: Device; onClose: () => void }) {
  ...
  useEffect(() => { getDeviceOverview(device.etiqueta).then(setOverview).catch(reason => setError(reason instanceof Error ? reason.message : 'No se pudo cargar el resumen.')); }, [device.etiqueta]);
    ...
    <Modal title={`Overview · ${getOperationalAlias(current) || current.etiqueta}`} onClose={onClose} wide>
      <div className="device-overview">
        ...
        <section className="device-overview-hero">
          ...
          <span className={`badge ${current.estado === 'Disponible' ? 'available' : current.estado === 'Prestado' ? 'loaned' : 'off'}`}>{current.estado || 'Sin revisar'}</span>
        </section>
