...
export interface PhotoPassEvent { id: number; tipo: string; persona: string; rol: string; motivo: string; operador: string; timestamp: string }
...
export const lendPhotoPass = (numero: number, payload: { persona: string; rol?: string; motivo?: string }) => apiSend<{ ok: true; item: PhotoPass }>(`/api/photo-passes/${numero}/lend`, 'POST', payload);
